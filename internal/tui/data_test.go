...
func TestActiveAndOpenCountsExcludeDoneAndCancelled(t *testing.T) {
	s := Snapshot{
		...
		Stories: []StoryRow{{Status: "todo"}, {Status: "done"}, {Status: "blocked"}},
		Tasks:   []TaskRow{{Status: "todo"}, {Status: "done"}, {Status: "blocked"}},
	}
	if got := s.EpicsActive(); got != 2 {
		t.Fatalf("EpicsActive = %d, want 2", got)
	}
	if got := s.StoriesOpen(); got != 2 {
		t.Fatalf("StoriesOpen = %d, want 2", got)
	}
...
func TestTasksForStoryFiltersAndSortsByID(t *testing.T) {
	s := Snapshot{Tasks: []TaskRow{
		{ID: 2, StoryID: 1}, {ID: 1, StoryID: 1}, {ID: 3, StoryID: 2},
	}}
	got := s.TasksForStory(1)
	if len(got) != 2 || got[0].ID != 1 || got[1].ID != 2 {
		t.Fatalf("TasksForStory(1) = %+v, want ids [1 2]", got)
	}
	if len(s.TasksForStory(99)) != 0 {
		t.Fatal("TasksForStory(99) should be empty")
	}
}
